    ...
    const getRestaurantList = async () => { 
        try {
            ...
            if(response.ok)
            {
                ...
                const normalData = restaurantData.filter(resItem => resItem?.card?.card?.['@type']==='type.googleapis.com/swiggy.presentation.food.v2.ItemCategory')
                console.log("normalData: ",normalData) //Recommended dishes data
                
                const nestedData = restaurantData.filter(resItem => resItem?.card?.card?.['@type']==='type.googleapis.com/swiggy.presentation.food.v2.NestedItemCategory') //rest all data
